...
struct rpgobj : g3d_callback, stats
{
    ...
    void render()
    {
        if(s_ai) 
        {
            ...
            renderclient(ent, model, NULL, ANIM_PUNCH, ent->lastaction, 0, ent->sink);
        }
        else
        {
            vec color, dir;
            lightreaching(ent->o, color, dir);  // FIXME just once for nonmoving objects
            rendermodel(color, dir, model, ANIM_MAPMODEL|ANIM_LOOP, 0, 0, ent->o.x, ent->o.y, ent->o.z-ent->eyeheight, ent->yaw+90, 0, 0, 0, ent);
        }
    }
