...
bool move(physent *d, vec &dir)
{
    ...
    vec obstacle;
    d->o.add(dir);
    if(!collide(d, d->type!=ENT_CAMERA ? dir : vec(0, 0, 0)) || (d->type==ENT_AI ? !collide(d) : (!dir.z && !collide(d, vec(0, 0, -1)))))
    {
        d->o = old;
...
void modifyvelocity(physent *pl, bool local, bool water, bool floating, int curtime)
{
    ...
    vec d(m);
    d.mul(pl->maxspeed);
    if(!water && !floating) d.mul((pl->move && !pl->strafe ? 1.3f : 1.0f) * (pl->physstate < PHYS_SLOPE && pl->move>=0 ? 1.3f : 1.0f)); // EXPERIMENTAL
