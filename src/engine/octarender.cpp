        ...
        if(!occluded || oe->query)
        {
            ...
            loopv(oe->mapmodels)
            {
                ...
                else if(e.visible)
                {
                    ...
                    if(&mmi) rendermodel(e.color, e.dir, mmi.name, anim, 0, mmi.tex, e.o.x, e.o.y, e.o.z, (float)((e.attr1+7)-(e.attr1+7)%15), 0, false, 10.0f, basetime, NULL, MDL_CULL_VFC | MDL_CULL_DIST);
                    e.visible = false;
                };
            };
            if(occluded)
