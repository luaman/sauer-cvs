...
struct animmodel : model
{
    ...
    void render(int anim, float speed, int basetime, const vec &o, float yaw, float pitch, dynent *d, modelattach *a, const vec &color, const vec &dir)
    {
        ...
        if(anim&ANIM_TRANSLUCENT)
        {
            ...
            glColorMask(GL_TRUE, GL_TRUE, GL_TRUE, renderpath!=R_FIXEDFUNCTION && reflecting && waterrefract && waterfade && hasFBO ? GL_FALSE : GL_TRUE);
    ...
    static void *lastvbuf, *lasttcbuf, *lastmtcbuf, *lastnbuf, *lastbbuf, *lastbdata;
