...
/*
    ...
    loopi(NUMRAYS)
    {
        ...
        && !SOLID(S(fast_f2nat(vx), fast_f2nat(vy))))       // try to avoid tracing ray if outside of frustrum
        {
            ...
            //float dx = (float)cos(angle); // improve sampling! needed? missed sample only causes slightly less occlusion
            //float dy = (float)sin(angle); // in fact we could increase sampling, at some cost in occlusion
            float sx = vx;
            float sy = vy;
            for(;;)
            {
