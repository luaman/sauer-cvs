...
struct monsterset
{
    ...
    struct monster : fpsent
    {
        ...
        monster(int _type, int _yaw, int _state, int _trigger, int _move, monsterset *_ms) : monstertypes(_ms->monstertypes), ms(_ms)
        {
            respawn();
            ...
            weight = t->weight;
            monsterstate = _state;
            if(state!=M_SLEEP) spawnplayer(this);
