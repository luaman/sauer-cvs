...
enum
{
    ...
    SV_EDITMODE, SV_EDITENT, SV_EDITF, SV_EDITT, SV_EDITM, SV_FLIP, SV_COPY, SV_PASTE, SV_ROTATE, SV_REPLACE, SV_DELCUBE, SV_NEWMAP, SV_GETMAP, SV_SENDMAP,
...
static char msgsizelookup(int msg)
{
    static char msgsizesl[] =               // size inclusive message token, 0 for variable or not-checked sizes
    {
        ...
        SV_TRYSPAWN, 1, SV_SPAWNSTATE, 13, SV_SPAWN, 2, SV_FORCEDEATH, 2, SV_ARENAWIN, 2,
...
{   
    ...
    void hitpush(int damage, const vec &dir, fpsent *actor, int gun)
    {
        ...
        if(gun==GUN_RL) push.mul(actor==this ? 5 : (type==ENT_AI ? 3 : 2));
        vel.add(push);
    }
